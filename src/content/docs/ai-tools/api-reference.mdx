...
**AI Gateway** covers OpenAI-compatible inference: chat completions, embeddings, images, audio, and model listing:

```text
https://ai.varity.app/v1
```

`https://varity.app/v1` remains a compatibility alias routed to the same service.
...
- **`/v1/*` is the OpenAI-compatible inference surface.** Chat completions, embeddings, images, audio, and model listing. Point any OpenAI client at it by setting the base URL. It has its own error shape and its own rate limits, documented in the [AI Gateway](/ai-gateway/) docs. Nothing on this page applies to it.

A single Varity API key does not automatically work on both surfaces: keys are scoped per product, and using one out of scope returns `403 credential_scope_violation`. Create AI Gateway keys from the Developer Portal **Inference keys** tab, which calls `POST /v1/keys` on `https://ai.varity.app` with your Portal session; that is the only AI Gateway key issuer (see [Billing and Keys](/ai-gateway/billing-and-keys/#account-routes)).
...
Send an `Idempotency-Key` on creates so a retry cannot produce a duplicate deployment:

- Deployment and template creates accept an optional key up to 160 characters.
- Machine create and delete require `[A-Za-z0-9._:-]{8,128}`.
- AI Gateway attachment mutations require `[A-Za-z0-9._:-]{1,160}`.
- Retention: 24 hours.
...
The header is optional on `POST /api/deployments` and
`POST /api/templates/{id}/deploy`; it is **required** on machine create/delete
and every `/api/ai-gateway/attachments` mutation. The operation-specific
OpenAPI parameter is authoritative.
...
A static site has no runtime, so it has no runtime logs. `GET
/api/deployments/{id}/logs` answers it with a non-retryable `409`
`runtime_not_started` ("This is a static site: it is served as files and has no
runtime, so there are no runtime logs."), not with an incomplete window to
retry. Its build and publish steps are the deployment run's events: read
`run_id` from `GET /api/deployments/{id}`, then `GET /api/deployments/runs/{run_id}`.
A deployment that never started a runtime (a failed create, a machine) gets the
same `409` with "No runtime logs: this deployment did not start a runtime."
...
GPU containers (`execution_class=container`) are not offered right now: the capability reads `temporarily_unavailable`, and a GPU container create or accelerator quote returns `409 capability_disabled`.
...
There are no per-route request budgets. A full deployment queue answers a
create with `429 queue_saturated` (retry with the same `Idempotency-Key`), and
the edge's per-client abuse limit answers a runaway client with a plain-text
`429`.
...
The live OpenAPI document at `https://varity.app/api/openapi.json` defines **34 paths and 46 operations** in the checked-in projection. The live document itself is authoritative.
...
| `GET` | `/api/deployments/{deployment_id}/access-credential` | Read owner-only private application access with a Portal session |
...
| `POST` | `/api/pricing/accelerator-quote` | GPU container quote; returns `409 capability_disabled` while GPU containers are unavailable |
