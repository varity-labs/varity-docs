...
| `privacy` | string | `"private"` on 42 models and `"anonymized"` on 40 models |
| `privacy_routes` | array | Read the available route classes from each model |
...
- **Need tool calling?** 30 models support it, and 25 support tool calls while streaming. See [Compatibility](/ai-gateway/compatibility/) for runtime capability checks.
