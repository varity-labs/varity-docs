...
The Varity MCP server (`@varity-labs/mcp@beta`) gives AI editors 16 tools for the full build and deploy workflow. It runs locally via stdio. No API keys or accounts needed to get started.
...
| `submit_to_store` | boolean | No | false | Legacy flag for a post-beta App Store surface |
...
12. **"Submit to the app store at $14.99"** → `varity_submit_to_store (post-beta, legacy flow)`
...
**"Cannot find module @varity-labs/mcp@beta"**
