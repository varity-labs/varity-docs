...
```text
{ "status": "healthy", "service": "varity-gateway", "version": "<deployed build>" }
```

The `version` field reports the deployed gateway build; re-probe for the current
one rather than pinning it in your own config.
