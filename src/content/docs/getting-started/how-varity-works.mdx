...
description: How Varity deploys supported source apps and runnable Docker services from project detection to a live URL.
---
...
Varity deploys supported Node, Python, Go, static, AI agent, and runnable Docker workloads with a fast, infrastructure-abstracted workflow. Source deployments do not require Docker setup; Docker image deployments are available when you already have a runnable HTTP service image.

## Who Varity is for

Varity works seamlessly with AI coding tools to build and ship apps. People reach for it in a few ways:

**Building something new with an AI tool**: You are using Claude Code, Cursor, or a similar tool to build an app from scratch. You want to go from "working locally" to "live URL" without leaving your editor. Varity's MCP server handles the entire deploy in one prompt.
...
In each case, Varity handles the deployment infrastructure. You just build and deploy.
...
2. **Open your project**
   ...
   Open a supported source project, or use a runnable Docker/OCI HTTP image for stacks Varity does not source-build yet.
...
**Framework detection:** Varity reads your `package.json`, `requirements.txt`, `pyproject.toml`, or Go module files to identify your framework and selects the right build process.
...
| `@aws-sdk/client-s3`, `boto3`, `minio`, S3-compatible config | Object storage |

You do not write connection strings or provision databases. Varity handles it.

**Environment variables:** Varity reads your `.env.varity`, `.env.local`, or `.env` file and passes those variables to your running app. Platform-specific variables left over from other hosting providers are filtered out automatically.

---

## Pricing

Varity pricing is based on reserved hardware profiles, while usage-metered cloud is billed by traffic, bandwidth, and invocations, so those bills grow as your app does. Your Varity bill is tied to your selected profile and does not change with traffic spikes, bandwidth, or invocation count.
...
| `varity_set_env` | Set environment variables for an app |
| `varity_redeploy` | Redeploy an existing app |
| `varity_deploy_agent` | Deploy a supported agent template |
