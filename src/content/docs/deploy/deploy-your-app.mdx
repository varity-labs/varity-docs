...
Supply the third-party secrets and configuration your application owns. Varity
does not create shared development credentials or an application ID variable.
When a supported attached service is actually provisioned, its reserved
connection variables are injected into the deployed runtime.

See [Environment Variables](/deploy/env-variables/) and
[Deployment Credentials](/deploy/managed-credentials/).
