...
3. **View your live app**
   ...
   ✓ Live: https://varity.app/your-app/
   ```
...
| Astro | `astro.config.*` | `npm run build` |
| Qwik | `qwik.config.*` | `npm run build` |
| Custom | `package.json` | `npm run build` |

Node servers (Express, Fastify, NestJS, Koa, Hono), Python apps (FastAPI, Django, Flask), Go modules, plain static HTML, and prebuilt Docker or OCI HTTP images deploy the same way. See [Supported Frameworks](/deploy/supported-frameworks/) for the full matrix.
...
Get detailed information about a deployment by app name:
...
<Accordion title="Advanced: where deployment state lives">

Varity owns your deployment state. The CLI does not write a local manifest for
a deploy, so `varitykit app list`, `app info`, and `app status` all read from
Varity and return the same answer on any machine.
