...
| `--name` | - | Custom app name. Static apps live at `https://varity.app/<name>/`; dynamic apps live at `https://<name>.varity.app/` |
| `--mode` | `auto` | Deployment mode (advanced) |
| `--target` | - | Deployment target |
| `--tier` | - | Hardware tier to reserve |
| `--repo-url` | - | Deploy directly from a GitHub repo URL |
| `--skip-build` | `false` | Skip the build step (deploy prebuilt output) |
| `--image` | - | Deploy a prebuilt OCI image instead of building from source |
| `--image-registry` | - | Registry host for a private `--image` |
| `--image-username` | - | Username for a private `--image` |
| `--image-password` | - | Password/token for a private `--image` |
