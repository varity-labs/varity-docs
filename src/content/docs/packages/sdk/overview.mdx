...
<Badge text="v2.0.0-alpha.1" variant="caution" />
...
<CardGrid>
  ...
  <Card title="Infrastructure" icon="rocket">
    Chain definitions, gas tracking, and thirdweb integration via subpath imports
  </Card>
</CardGrid>
...
| Export | Type | Description |
|--------|------|-------------|
| `VERSION` | const (string) | SDK version (`'2.0.0-alpha.1'`) |
...
<Aside type="caution">
**Alpha release** -- The main SDK entry point currently exports database and credential utilities only. Additional modules (storage, analytics, notifications) are planned for future releases. See the subpath imports below for infrastructure access.
</Aside>
...
The current SDK (v2.0.0-alpha.1) uses a modular approach:
