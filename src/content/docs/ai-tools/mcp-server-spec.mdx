...
### `varity_set_env`: Set environment variables

Set or update environment variables on an existing deployment, then redeploy it in place on the same URL and reserved hardware.

| Parameter | Type | Required | Default | Description |
|-----------|------|----------|---------|-------------|
| `name` | string | Yes | none | Existing deployment name, the slug in its Varity URL |
| `env` | object | Yes | none | Environment variables as key-value pairs, for example `{ "OPENAI_API_KEY": "sk-..." }` |
| `replace` | boolean | No | `false` | Replace the full env set instead of merging with existing variables |

**Annotations:** `destructiveHint: true` (updates live deployment config)

**Example prompt:** "Set OPENAI_API_KEY on my-api and redeploy it"

---

### `varity_redeploy`: Redeploy or restart in place

Redeploy or restart an existing app in place. The deployment keeps the same URL and reserved hardware.
...
**Example prompt:** "Redeploy my-api in place"

---

### `varity_list_agents`: List AI agent templates

List the curated AI agent templates Varity can deploy from the MCP.

*No parameters required.*

**Annotations:** `readOnlyHint: true`

**Example prompt:** "What AI agents can Varity deploy?"

---

### `varity_agent_info`: Show agent template details

Show the required environment variables, ports, resource profile, access mode, and notes for one agent template.

| Parameter | Type | Required | Default | Description |
|-----------|------|----------|---------|-------------|
| `name` | `"hermes"` \| `"openclaw"` \| `"agent-zero"` \| `"autoresearch"` \| `"eliza-venice"` | Yes | none | Agent template slug |
...
### `varity_deploy_agent`: Deploy an AI agent template

Deploy one curated AI agent template. Use this for templates such as Agent Zero instead of `varity_deploy`, which is for deploying a local project or Docker image.

| Parameter | Type | Required | Default | Description |
|-----------|------|----------|---------|-------------|
| `agent` | `"hermes"` \| `"openclaw"` \| `"agent-zero"` \| `"autoresearch"` \| `"eliza-venice"` | Yes | none | Agent template slug |
| `name` | string | No | generated name | Memorable deployment name for the URL slug |
| `env` | object | No | none | Required environment variables for the selected agent |
...
10. **"Redeploy this app"** → `varity_redeploy` (when you need an in-place restart)
11. **"Tear down this deployment"** → `varity_delete_deployment` (when you no longer need it)

**Deploying Agent Zero or another template:**

1. **"What AI agents can Varity deploy?"** → `varity_list_agents`
2. **"Show details for Agent Zero"** → `varity_agent_info`
3. **"Deploy Agent Zero"** → `varity_deploy_agent`
