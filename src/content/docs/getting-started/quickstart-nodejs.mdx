...
```bash
VARITY_DATABASE_URL=https://your-database.example.com
...
Legacy setups may still use the older database environment variable name; prefer `VARITY_DATABASE_URL` for all new projects.
</Aside>
...
3. **App Store status (post-beta)** (optional)
