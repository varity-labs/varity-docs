...
The Varity MCP server (`@varity-labs/mcp`) gives AI editors a full set of tools for the full build, deploy, and operate workflow. It runs locally via stdio. Install it from the current npm package.
...
### `varity_set_env`: Update environment variables
...
| `env` | object | Yes | none | Environment variables as `{ "NAME": "value" }`. Names must be `UPPER_SNAKE_CASE`. |
| `replace` | boolean | No | `false` | When true, replace the full variable set instead of merging over the existing set. |

**Example prompt:** "Set STRIPE_SECRET_KEY on my dashboard app"

---

### `varity_redeploy`: Redeploy or restart in place

Redeploy or restart an existing app in place. Use this to pull the latest image, rebuild from the configured source, or recover a stuck deployment without creating a new URL.

| Parameter | Type | Required | Default | Description |
|-----------|------|----------|---------|-------------|
| `name` | string | Yes | none | The app name / subdomain — the `<name>` in `https://varity.app/<name>/` |

**Example prompt:** "Redeploy my-api"
...
8. **"Set DATABASE_URL on this app"** → `varity_set_env` (if configuration changes)
9. **"Redeploy this app"** → `varity_redeploy` (if you need to restart or pull the latest source/image)
10. **"Show me the build logs"** → `varity_deploy_logs` (if needed)
