...
3. **GPU workloads**: GPU VMs and GPU bare metal; you pick the exact GPU offer from the marketplace.
...
**Auto-wired services** (provisioned in the same deployment when your code needs them): Postgres (with pgvector), Redis, MongoDB, MySQL, and object storage. See [Auto-wired Services](/deploy/auto-wired-services/).
...
See [CPU virtual machines](/machines/cpu-vms/) for live catalog and quote instructions.

### GPU workloads

GPU compute comes in two forms: **GPU VMs** and **GPU bare metal**. GPU
containers are not offered right now. The GPU marketplace shows the exact cards
on offer. You
