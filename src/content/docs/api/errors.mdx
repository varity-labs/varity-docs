...
| <span id="invalid_idempotency_key"></span>`invalid_idempotency_key` | 400 | no | The key does not satisfy the called route's OpenAPI parameter. Machine operations use `[A-Za-z0-9._:-]{8,128}`; deployment and attachment routes have different bounds. |
...
| <span id="capability_disabled"></span>`capability_disabled` | 409 | no | The requested capability is not offered right now (GPU containers). |
...
| Code | HTTP | Retryable | Cause |
|------|------|-----------|-------|
| <span id="queue_saturated"></span>`queue_saturated` | 429 | yes | The platform-wide deployment queue is full. Nothing was allocated or charged. |

Wait a few seconds and retry with the same `Idempotency-Key`. A plain-text `429` without this envelope comes from the edge's abuse protection instead. See [Rate Limits](/api/limits/).
...
The exception is a deployment with no runtime at all, such as a static site: its log read is a definite, non-retryable `409` `runtime_not_started` rather than an incomplete window, because retrying can never produce runtime logs. See [Public API Reference](/ai-tools/api-reference/).
