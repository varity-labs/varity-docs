...
## Runnable Public Docker Images

If your app is already packaged as a Docker image, framework detection does not apply. Varity can run public Docker images that start a healthy HTTP service on the configured port:
...
<CardGrid>
  <Card title="Go modules" icon="rocket">
    Go services with a `go.mod` file. Varity builds the module at deploy time and runs the resulting HTTP service.
...
Varity currently supports: Next.js, Astro, React, Vue, Node.js (Express/Fastify/NestJS/Koa/Hono), Python (FastAPI/Django/Flask), Go, and static HTML.
