...
| Endpoint | Status |
|----------|--------|
| `GET /health` | **Supported.** Public. Returns `{"status":"healthy","service":"varity-gateway","version":"<deployed build>"}` (the version tracks the deployed build; re-probe for the current one). |
