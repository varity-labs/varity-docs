...
**Migrating from another platform**: You have an existing app on Vercel and want lower costs. Run `varitykit migrate` to remove platform-specific config, then deploy. Most migrations take under 5 minutes.

**Starting from a template**: You want a production-ready starting point with auth, a database, and UI components already connected. Run `varity_init` and get a working app scaffold in seconds. This path is currently experimental during beta.
