...
| Endpoint | Status |
|----------|--------|
| `GET /health` | **Supported.** Public. Returns `{"status":"healthy","service":"varity-gateway","version":"1.14.12"}`. |
...
52. The private models reporting `tool_calls: true` are listed below; query the
catalog for the complete current set, including anonymized models.
...
Four of those private models support tool calls only in non-streaming mode
...
is outside the supported surface. Drop `stream` or select a model whose live
catalog entry reports `streaming_tool_calls: true`.
