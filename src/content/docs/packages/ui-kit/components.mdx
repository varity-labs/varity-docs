...
<Accordion title="Advanced: Smart Account Components">

**Varity manages these credentials automatically. Only use this if you need custom configuration.**

These components provide direct access to smart account functionality. Most apps don't need these — `PrivyStack` handles account creation automatically.

### SmartWalletProvider

Provides smart account functionality with free operations.
...
Button to connect or display smart account.

```tsx
import { SmartWalletConnectButton } from '@varity-labs/ui-kit';
...
Shows green badge when free operations are enabled.

```tsx
import { GaslessBadge } from '@varity-labs/ui-kit';
