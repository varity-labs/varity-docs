...
Your app is hosted on Varity's infrastructure with automatic SSL. Static deployments are served over a global CDN; dynamic apps run as containers in a single region.
...
No. Cancel anytime, no lock-in, no exit fees, and no minimum spend on hosting. The AI Gateway is prepaid and has its own minimum refill; see [Pricing and Costs](/resources/pricing/).
