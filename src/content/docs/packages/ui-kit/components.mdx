...
Full-featured provider for dashboards. Includes Privy auth, Thirdweb, React Query, and wallet sync.
...
| Prop | Type | Default | Description |
|------|------|---------|-------------|
| `privyAppId` | `string` | env var | Privy App ID |
...
Lower-level Privy wrapper. Most apps should use `PrivyStack` instead.
...
### Re-exported Privy Hooks

For convenience, these hooks from `@privy-io/react-auth` are re-exported:
...
const { pay, status } = useVarityPayment({ appId: '...' });
```
...
| `usePrivy` | `@varity-labs/ui-kit` | Privy auth state (`authenticated`, `user`, `ready`) |
