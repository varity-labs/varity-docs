...
<<<<<<< HEAD
description: "@varity-labs/sdk provides a zero-config database, credential management, and USDC utilities. The core building block for all Varity apps."
=======
...
| `isProductionCredentials` | function | `(appId?) => boolean` -- check if using production credentials |
| `getCredentialWarning` | function | `(environment) => string \| null` -- get a warning message if credentials need attention |
| `logCredentialUsage` | function | `(appId?) => void` -- log which credentials are being used |
=======
