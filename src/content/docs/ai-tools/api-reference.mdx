...
Both URLs serve the same OpenAPI 3.1 document encoded as JSON. The docs
mirror keeps the historical `.yaml` path for compatibility; JSON is valid
YAML, so either extension parses, but point strict JSON tooling at it
directly rather than expecting YAML syntax. `varity.app` is authoritative;
the docs copy is a mirror.
...
- Format: `[A-Za-z0-9._:-]`, up to 160 characters on deployment and template creates. `POST /api/machines` and `DELETE /api/machines/{machine_id}` take 8 to 128 characters. Anything else returns `400 invalid_idempotency_key`.
- Retention: 24 hours.
...
The header is optional on `POST /api/deployments` and `POST /api/templates/{id}/deploy`, and **required** on `POST /api/machines`, `DELETE /api/machines/{machine_id}`, and every `/api/ai-gateway/attachments` mutation.
