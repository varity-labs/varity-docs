...
Credit card payments via Stripe are now available. Users can purchase deploy keys and subscriptions using a credit card. Purchase a deploy key at [varity.so](https://varity.so) to get started.
</Aside>
