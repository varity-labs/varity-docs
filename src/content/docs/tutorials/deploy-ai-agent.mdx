...
If you prefer the terminal or your AI editor, you can deploy any of these tools by its prebuilt Docker image:

<Tabs>
  <TabItem label="CLI">

  Requires `varitykit` 2.0.6 or later. Deploy an agent tool by its published image, passing any required environment variables with repeated `--env` flags:

  ```bash
  varitykit app deploy --image flowiseai/flowise --env FLOWISE_USERNAME=... --env FLOWISE_PASSWORD=...
  ```

  </TabItem>
  <TabItem label="MCP (AI coding tools)">

  With the [Varity MCP server](/ai-tools/mcp-server-spec) installed, ask your AI editor to use the agent-template tools:

  > "Show details for Agent Zero, then deploy Agent Zero on Varity"

  The MCP should call `varity_agent_info` first, then `varity_deploy_agent` with `agent: "agent-zero"`. Use `varity_deploy` for a local project or Docker image.
