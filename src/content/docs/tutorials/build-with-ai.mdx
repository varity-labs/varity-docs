...
9. **Monetize your app**
   ...
   > "Submit my app to the App Store (post-beta) for $29 per month"

   **What happens:** The AI runs `varity_submit_to_store (post-beta)` to:
   - List your app on [store.varity.so (post-beta)](https://store.varity.so)
...
| "Submit to store" | `varity_submit_to_store (post-beta)` | Lists app, sets up billing |
