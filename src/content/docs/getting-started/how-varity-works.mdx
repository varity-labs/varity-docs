...
| [**developer.store.varity.so (post-beta)**](https://developer.store.varity.so) | Developer portal: deploy, monitor, billing |
| [**store.varity.so (post-beta)**](https://store.varity.so) | User app store (coming post-beta) |
...
| `varity_submit_to_store (post-beta)` | Legacy tool for a post-beta App Store surface |
