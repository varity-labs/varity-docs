...
description: "Configure environment variables for Varity deployments. Inject custom API keys via .env.varity. Most Varity credentials are managed automatically."
---
...
| `DATABASE_URL`, `REDIS_URL`, `MONGODB_URI`, `MYSQL_URL`, `OLLAMA_URL` | Varity service wiring |
| `POSTGRES_USER`, `POSTGRES_PASSWORD`, `POSTGRES_DB` | Auto-wired database config |
...
If you attach the [Varity AI Gateway](/ai-gateway/) to a deployment, Varity injects `VARITY_AI_API_KEY` and `VARITY_AI_BASE_URL` into its runtime environment for you. You do not create or set them yourself. See [Authentication from an app deployed on Varity](/ai-gateway/#from-an-app-deployed-on-varity).
