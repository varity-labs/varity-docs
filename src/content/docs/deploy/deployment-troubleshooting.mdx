...
`varitykit app info my-app` shows a deployment's name, type, URL, and timestamps. The CLI has no log command, so read past logs from the dashboard, from the MCP server, or from the API:
...
|-----------|------|----------|-------------|
| `deployment_id` | string | Yes | The deployment ID, from the dashboard or from `GET /api/deployments` |
| `limit` | number | No | Max log lines (default 100) |

<Aside type="tip" icon="approve-check">
Don't know the deployment ID? Call `varity_deploy_status` from your AI editor, or read `GET /api/deployments`. `varitykit app list` shows app names rather than deployment IDs.
</Aside>
...
If `doctor` passes, re-deploy. If the error recurs, report it to [support@varity.so](mailto:support@varity.so) with the full output of `varitykit app info <app-name>`.
...
2. **GitHub**: [varity-labs](https://github.com/varity-labs). Include the output of `varitykit app info <app-name>`
