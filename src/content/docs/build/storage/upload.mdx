...
<Aside type="note">
**File uploads (images, videos, documents)** are not yet available through the SDK. The current storage API is for structured JSON data. File storage via IPFS/Filecoin is planned for a future release. In the meantime, you can store file URLs from external services (e.g., Cloudinary, S3) as string fields in your records.
</Aside>
