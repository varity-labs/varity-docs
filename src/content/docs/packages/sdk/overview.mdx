...
description: "@varity-labs/sdk provides a zero-config database, credential management, and infrastructure utilities. The core building block for all Varity apps."
---
...
import type {
  ...
  DatabaseConfig,     // { databaseUrl?: string, appToken?: string }
...
For apps that need direct access to advanced infrastructure configuration, the SDK provides subpath imports. These are separate from the main entry point and are primarily used by the UI Kit and CLI internally.

### Chains (`@varity-labs/sdk/chains`)

Chain definitions for Varity L3, Arbitrum, and Base networks, plus stable amount formatting utilities.
...
import {
  ...
  AMOUNT_DECIMALS,
  VARITY_TOKEN_ADDRESS,
  SUPPORTED_CHAINS,
} from '@varity-labs/sdk/chains';

// Format a raw stable amount amount (6 decimals) for display
const display = formatAmount(BigInt(1500000)); // "1.50"

// Parse a human-readable amount to raw
const raw = parseAmount('25.00'); // BigInt(25000000)
...
Track gas usage, calculate costs in stable amount, and manage billing cycles.
