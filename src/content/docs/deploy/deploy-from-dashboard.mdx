...
- Your app's source: a public GitHub repo, a connected GitHub account, or a Docker image reference.
...
2. **Choose your source**
   ...
   - **Docker image:** provide an image reference (for example `nginx:latest` or `ghcr.io/acme/api:1.4`). A private image also needs registry credentials; see [Deploy a Docker Image](/deploy/deploy-docker-image/). This is the path for any language Varity doesn't build from source yet.
...
- A predictable bill: up to a fixed monthly maximum per app, prorated by running time. See [How Pricing Works](/resources/pricing/)
