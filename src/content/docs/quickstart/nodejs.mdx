...
2. **Read (Get Documents)**
   ...
   async function getAllUsers() {
     ...
     // Returns: [{ id: 'doc_123', data: {...} }, ...]
   }

   // Get by ID
   async function getUserById(id: string) {
     const user = await db.collection<User>('users').getById(id);
...
app.get('/api/users/:id', async (req, res) => {
  try {
    const user = await db.collection<User>('users').getById(req.params.id);
...
async function safeGetUser(id: string) {
  try {
    const user = await db.collection('users').getById(id);
...
// Filter by role
const admins = allUsers.filter((user) => user.data.role === 'admin');

// Filter by email domain
const companyUsers = allUsers.filter((user) =>
  user.data.email.endsWith('@company.com')
);

// Combine filters
const recentAdmins = allUsers
  .filter((user) => user.data.role === 'admin')
  .filter((user) => {
    const created = new Date(user.data.createdAt);
...
// TypeScript knows user.data is User
users.forEach((user) => {
  console.log(user.data.name); // ✓ Type-safe
  console.log(user.data.invalid); // ✗ TypeScript error
});
```
...
```typescript
async function getUser(id: string): Promise<{ id: string; data: User }> {
  return await db.collection<User>('users').getById(id);
}

async function getAllUsers(): Promise<Array<{ id: string; data: User }>> {
...
<Aside type="tip">
The CLI detects your Node.js/Express app automatically and deploys it to Akash (decentralized compute). Your database credentials are injected at build time.
</Aside>
