...
description: Recover after a bad Varity deploy by redeploying a known-good version under the same app name.
---
...
<Aside type="note">
**Rollback to a previous deployment is not available yet.** The CLI still carries a `varitykit app rollback` command, but it reads a local deployment history that the current deploy path never writes, so it cannot restore an app deployed with the current CLI. There is no rollback endpoint in the platform API and no restore button in the dashboard. To recover, redeploy a working version as described below.
</Aside>
...
Inspect a specific deployment's details by app name:
