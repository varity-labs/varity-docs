...
```bash
varitykit app status deploy-1737491000
```

## Custom Domains

Add your own domain to your Varity app:

```bash
varitykit domain add your-domain.com
```

Follow the prompts to configure DNS. Varity will automatically provision SSL certificates.
...
Most users do not need these settings. Use defaults unless you have specific requirements.
:::
...
```bash
varitykit app deploy --build-command "pnpm build"
```
...
```bash
varitykit app deploy --output-dir "dist"
```

### Resource Allocation (Dynamic Apps)

For dynamic apps, configure resources:
