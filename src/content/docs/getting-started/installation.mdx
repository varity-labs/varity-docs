...
<Tabs>
  <TabItem label="Claude Code">
    ```bash
    claude mcp add varity -- npx -y @varity-labs/mcp@beta@beta
    ```
...
1. **Install the CLI**
   ...
   If you need a fallback in a constrained environment, use:

   ```bash
   python -m pip install --user varitykit
   ```
...
## Option 3: Legacy SDK packages (not part of the current product surface)

<Aside type="caution">
`@varity-labs/sdk`, `@varity-labs/ui-kit`, and `@varity-labs/types` are legacy packages. The current shipped product surface is `varitykit` (PyPI) plus `@varity-labs/mcp@beta` (npm).
</Aside>

If you are following current deployment docs, skip this section.
