...
| Class | Covers |
|-------|--------|
| Read | Listing and reading deployments, machines, templates, credits, profiles, capabilities, keys, attachments |
| Quote | `/api/pricing/estimate`, `/api/pricing/accelerator-quote`, `/api/pricing/machine-quote` |
| Logs | `/api/deployments/{id}/logs` and `/api/deployments/{id}/events` |
| Stop | Deleting deployments and machines, revoking API keys |
| Deploy create | `POST /api/deployments`, `POST /api/templates/{id}/deploy`, `POST /api/machines` |
| Mutation | Redeploy, restart, env updates, webhooks, key creation, AI Gateway attachments |
...
Limited routes return standard `RateLimit-*` headers. Program against them
rather than copying a numeric budget from documentation. The headers are
authoritative for your account at that moment.
...
The AI Gateway (`https://ai.varity.app/v1`) is a separate surface with its own independent rate-limit, concurrency, and budget admission controls. Limits documented here do not apply to it, and its headers differ. See [AI Gateway Rate Limits](/ai-gateway/limits/).
...
- **[AI Gateway Rate Limits](/ai-gateway/limits/)**: the separate budgets for `ai.varity.app/v1`
