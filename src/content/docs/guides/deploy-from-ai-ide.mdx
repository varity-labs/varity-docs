...
5. **Deploy to production**
   ...
   The AI calls `varity_build` and `varity_deploy`, then reports the live URL when the deploy finishes.
...
Hosted OAuth and per-user deployment authorization are not certified end to end yet. Until that verification passes, use the stdio package (`npx -y @varity-labs/mcp`) for deploys, environment changes, and deletes.
