...
description: Deploy a runnable Docker HTTP service to Varity from the CLI, the Developer Portal, or your AI coding tool. No repo and no build step required.
---
...
If your app is already packaged as a Docker image, you can deploy it to Varity directly. No repository, no build step — Varity pulls the image and runs it as a web service.

This is also the path for languages Varity does not build from source yet (Rust, Ruby, and others): package your app as an image and deploy it like any other HTTP service container.

```bash
varitykit app deploy --image nginx:latest
```

The image must start a healthy HTTP service on the configured port. Raw model artifacts, one-shot jobs, and images with no long-running web process need a serving wrapper first.
...
<Tabs>
  ...
  4. For a private registry image, add the registry host, username, and password/token
  5. Click deploy

  The portal supports public and private registry images. Private registry credentials are forwarded only for the image pull.
...
| Private (credentials required) | ✅ `--image-registry` / `--image-username` / `--image-password` | ✅ `image_credentials` | ✅ Registry credentials |
