...
description: Deploy a GitHub repo, a Docker image, or a ready-made template from the Varity developer dashboard — no command line required.
---
...
- Your app's source: a supported public GitHub repo, or a prebuilt public/private Docker image that starts an HTTP service.
...
2. **Choose your source**
   ...
   Pick one of:
   - **GitHub repo** — paste a public repository URL, or connect your GitHub account and browse repos where supported.
   - **Docker image** — provide a public or private image reference (for example `nginx:latest` or `ghcr.io/acme/api:1.4`). Private registry images require pull credentials. This is also the path for languages Varity doesn't build from source yet.
...
- [Deploy a Docker Image](/deploy/deploy-docker-image/) — deploy a runnable HTTP service image.
