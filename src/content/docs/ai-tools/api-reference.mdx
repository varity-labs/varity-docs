...
Rate limits are enforced per account and per class of request. Exhausting a
budget returns `429 rate_limited` with a `Retry-After` header. Honor the
returned `RateLimit-*` headers; documentation does not own a copied numeric
budget.
